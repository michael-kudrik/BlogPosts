---
title: "Issue 1"
date: "2023-11-29"
tags: ["Weekly"]
---
  
# Issue 1 - The First of Many 
  
## trendy tools  

### Obsidian.md
https://obsidian.md/

Obsidian is a customizable note taking application that utilizes markdown files. I quite like this method of taking notes with markdown as it allows easy formatting. 

[<CustomImage src="https://raw.githubusercontent.com/michael-kudrik/BlogPosts/main/images/ObsidianExample.png" alt="Image of Obsidian Application"/>]: #

### Activity Watch
https://activitywatch.net/

Activity watch is an open-source alternative to time trackers such as WakaTime. It is useful to those who want to monitor time spent on their devices. 
![](https://activitywatch.net/img/screenshots/screenshot-v0.8.0b9-timeline.png)

### AppFlowy
https://www.appflowy.io/

AppFlowy is another great open source alternative to a popular application. It works much like Notion and acts as a centralized place for tasks, notes, and projects. 
![](https://www.appflowy.io/_next/image?url=https%3A%2F%2Fd3uafhn8yrvdfn.cloudfront.net%2Fwebsite%2Fproduction%2F_next%2Fstatic%2Fmedia%2Fdesktop.d93f3d0b.png&w=3840&q=75)

## technology related

### Interactive Particles with Three.js
https://github.com/brunoimbrizi/interactive-particles

I came across this detailed article on how to display an image as particles that move when the mouse hovers over it. It is a bit complex for me at this time but I would love to try it out in the future. 
![](https://user-images.githubusercontent.com/880280/51060802-de702c00-15e8-11e9-9bff-58b02dc284a3.jpg)

### Medusa JavaFX Gauges Library
https://github.com/HanSolo/medusa

Recently, we have been working with JavaFX withing my intro to computer science class. While browsing github, I found this interesting gauge library which can be incorporated. The creator of this library has many other JavaFX related things, so check him out.
![](https://github.com/HanSolo/Medusa/raw/master/Overview.png?raw=true)

